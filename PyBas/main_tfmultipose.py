...
# Create a UDP socket
udp_ip = "127.0.0.1"  # IP address to send data to
...
def visualize_pose_multipose(frame, keypoints_with_scores, confidence_threshold=0.2):
    ...
    for person in keypoints_with_scores[0]:
        ...
        for i, kp in enumerate(keypoints):
            if kp[2] > confidence_threshold:  # Only draw keypoints with a confidence score above the threshold
                x = int(kp[1] * frame.shape[1])
                y = int(kp[0] * frame.shape[0])
                cv2.circle(frame, (x, y), 6, (255, 0, 0), -1)
                person_keypoints[keypoint_names[i]] = {"x": float(kp[1]), "y": float(kp[0]), "confidence": float(kp[2])}
        ...
        for connection in connections:
            ...
            if start[2] > confidence_threshold and end[2] > confidence_threshold:  # Only draw connections with confidence scores above the threshold
                start_point = (int(start[1] * frame.shape[1]), int(start[0] * frame.shape[0]))
                end_point = (int(end[1] * frame.shape[1]), int(end[0] * frame.shape[0]))
                cv2.line(frame, start_point, end_point, (0, 0, 255), 2)
    
    # Convert keypoints to JSON and send over UDP
    keypoints_json = json.dumps({"keypoints": keypoints_to_send})
    pprint.pprint(keypoints_json)
    sock.sendto(keypoints_json.encode(), (udp_ip, udp_port))
...
while cap.isOpened():
    ...
    frame_with_poses = visualize_pose_multipose(frame, keypoints_with_scores, confidence_threshold)
